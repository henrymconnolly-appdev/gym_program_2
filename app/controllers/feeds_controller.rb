class FeedsController < ApplicationController
  def index
    ...
    @feeds = @q.result(:distinct => true).includes(:user, :totals, :subscriptions).page(params[:page]).per(10)
  ...
  def create_row
    ...
    @feed.subscribed_lift_totals = params.fetch("subscribed_lift_totals")
    @feed.subscribed_lift_dates = params.fetch("subscribed_lift_dates")
    @feed.subscribed_lift_username = params.fetch("subscribed_lift_username")
